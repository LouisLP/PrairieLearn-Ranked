...
CREATE
OR REPLACE FUNCTION move_to_seasonal_session () RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    ...
    IF NEW.is_live = FALSE THEN
        ...
        LOOP
            ...
            IF EXISTS (
                SELECT 1 
                FROM PLR_seasonal_session_credentials
                JOIN PLR_seasonal_session ON PLR_seasonal_session_credentials.session_id = PLR_seasonal_session.id
                WHERE PLR_seasonal_session.course_instance_id = NEW.course_instance_id AND user_id = cred.user_id
            ) THEN
            ...
            ELSE
                ...
                VALUES ((SELECT id FROM PLR_seasonal_session WHERE course_instance_id = NEW.course_instance_id), cred.user_id, cred.points, cred.duration);
            END IF;
        END LOOP;
