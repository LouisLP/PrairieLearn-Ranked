CREATE
OR REPLACE FUNCTION half_baked_achievement () RETURNS TRIGGER AS $$
BEGIN
  ...
  WITH half_max_users AS (
    ...
    WHERE plr_live_session.is_live = FALSE
      AND ROUND(plr_live_session_credentials.points / 3628) = ROUND(assessments.max_points / 2)
  )
  ...
  WHERE NOT EXISTS (
    ...
    WHERE plr_has_achieved.user_id = half_max_users.user_id
      AND plr_has_achieved.achievement_id = 8
  );
...
CREATE TRIGGER half_baked_achievement_trigger
AFTER
UPDATE OF is_live ON plr_live_session FOR EACH ROW WHEN (NEW.is_live = FALSE)
EXECUTE FUNCTION half_baked_achievement ();
